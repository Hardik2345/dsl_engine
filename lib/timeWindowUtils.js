...
function listHourlyLandingPagePathUnsupportedFilters(filters = []) {
  return (Array.isArray(filters) ? filters : [])
    .filter((filter) => filter?.dimension && filter.dimension !== 'product_id')
    .map((filter) => filter.dimension);
}
...
function normalizeRerunContext(context, timeZone = BUSINESS_TIME_ZONE) {
  ...
  // Tenant branding is injected into a run immediately before execution. Do
  // not carry that injected snapshot into a rerun, otherwise later branding
  // edits are ignored because the runner sees branding already present.
  const { emailBranding: _previousEmailBranding, ...rerunMeta } = context.meta || {};

  return {
    meta: {
      ...rerunMeta,
      timezone: context.meta?.timezone || timeZone,
    ...
    // A completed run contains derived values from its previous execution.
    // Preserve only user-supplied filters when rerunning; analytical state must
    // be recomputed from scratch for the selected workflow version.
    ...
    scratch: {},
    executionTrace: [],
    // Design doc §10.2: a rerun must not resume mid-flight state-machine tracking
    // from the prior execution -- these buckets don't exist until Phase 3's
    // alert_state node, but clearing them here now avoids a second rerun-hardening
    // pass being needed once workflows actually populate them.
    alertStates: {},
    notifications: []
  };
}
